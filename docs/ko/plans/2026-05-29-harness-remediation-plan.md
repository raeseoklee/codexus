...
날짜: 2026-05-29
상태: accepted. Remediation pass에서 R1, R2a, R2b, R3, R4, R5, R6, R7, R8을
구현했고, repair context redaction도 확장했습니다.
...
닫았습니다. 외부 프로세스가 다른 프로세스의 run을 취소하는 `cx cancel <run-id>`도
owner heartbeat와 cancel-marker protocol로 구현했습니다.
...
- Secret-like token은 확장된 redaction hook을 거쳐 저장/주입됩니다. API token,
  AWS key, JWT, key/value secret assignment, `.env` dump, PEM/private-key block을
  대표 패턴으로 막습니다.
...
각 active run은 pid, hostname, heartbeat, TTL이 들어 있는 `owner.json`을 씁니다.
`cx cancel`은 live owner가 있으면 `cancel-request.json` marker만 쓰고, owning
kernel이 marker를 polling해 local `AbortSignal`을 중단한 뒤 terminal `cancelled`
ledger를 직접 기록합니다. Owner가 dead/stale이면 canceller가 `run.cancel_orphaned`
및 `run.terminal` event를 쓰고 orphan ledger를 `cancelled`로 닫습니다.
...
1. Usage event shape fixture를 실제 Codex CLI 변화에 맞춰 계속 보강.
2. Evolution replay fixture를 새 canonical scenario가 추가될 때마다 확장.
3. Tool/MCP/app-server live path는 policy/approval contract가 더 단단해질 때까지
   gated 상태를 유지.
