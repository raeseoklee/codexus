...
- `0.1.x` patch release는 stable JSON field를 additive로만 추가할 수 있습니다.
- Experimental/deferred surface는 자기 stability를 보고하고 frozen stable contract 밖에
  머무는 경우 `0.1.x`에서 추가할 수 있습니다.
...
따라서 큰 experimental bundle도 frozen stable contract를 바꾸지 않으면 `0.1.x`일 수
있습니다. 반대로 작은 breaking contract change는 `0.2.0`이 필요합니다.
