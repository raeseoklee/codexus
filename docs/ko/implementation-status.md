...
- `locks list/inspect/clear`, `schema check`, `app-server status/roundtrip`
...
- bounded active skill/memory retrieval과 prompt-safe context formatting
...
- app-server schema fixture/status/dry-run roundtrip, live execution disabled
- cron/gateway disabled feature gate와 dry-run automation plan
- config/state/event/memory/skill versioned schema artifact
...
- init, observability, active skill index/export, adapter retrieval/context formatting, gated model replay, stale lock, schema artifact, driver-failure repair, app-server dry-run, memory lifecycle, packaging, feature gate 테스트
...
- app-server driver는 live execution disabled이며 fixture/status/dry-run roundtrip probe만 구현했습니다.
...
- config/schema validation은 runtime validation과 schema artifact 수준이며 full external JSON Schema enforcement는 아직 아닙니다.
