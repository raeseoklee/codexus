...
[release evidence](../release-evidence/0.1.14.md)를 기준으로 확인하세요.

## Baseline

- 현재 published baseline: `0.1.14`.
- Stable execution path: 로컬에서 인증된 `codex exec --json`을 Codexus가 감독하는 경로.
- Stable management expectation: stable JSON field는 `0.1.x` 동안 frozen 상태를 유지하고,
  experimental surface는 promotion 없이 추가될 수 있습니다.
