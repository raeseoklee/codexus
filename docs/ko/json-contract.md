...
- Release integrity local output(`release check --gate`, `--live` 없음):
  ...
  trusted-publishing workflow shape, trusted-publishing workflow의 post-publish
  dist-tag mutation 금지, pinned publish actions, installer asset workflow wiring,
  redacted release-evidence docs를 포함합니다. GitHub/npm post-publish live
