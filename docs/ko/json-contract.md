...
- Release integrity local output(`release check --gate`, `--live` 없음):
  ...
  비파괴 installer help parsing, trusted-publishing workflow shape,
  trusted-publishing workflow의 post-publish dist-tag mutation 금지, pinned publish
  actions, installer asset workflow wiring, redacted release-evidence docs를
  포함합니다. GitHub/npm post-publish live sign-off는 계속 명시적 opt-in이 필요한
  experimental surface입니다. Stable release에서 사용할 경우 npm `latest`,
  GitHub latest, installer asset identity를 gate합니다. npm `next`는 명시적
  prerelease publish용으로 남으며 stable release completion gate가 아닙니다.
  구조화된 `nextDistTagAction`은 compatibility를 위해 계속 존재하고 stable
  sign-off에서는 `not_applicable`을 보고합니다.
