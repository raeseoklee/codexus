...
1. [raeseoklee/claw-code](https://github.com/raeseoklee/claw-code)
   ...
   - 현재 접근 메모: 2026-05-29 기준 GitHub metadata는 읽을 수 있었지만,
     clone은 `Your repository is disabled`와 HTTP 403으로 실패했습니다.
     필수 레퍼런스로 유지하되, source-level 주장은 repository를 다시 읽을 수
     있을 때까지 provisional로 표시합니다.
...
- CLI/session parity: `raeseoklee/claw-code`에서 시작하고,
...
7. 영어 문서와 대응되는 `docs/ko/...` 문서를 함께 업데이트합니다.
