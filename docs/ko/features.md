...
| Release integrity | `codexus release check --gate --json` | install script, trusted-publishing workflow, release evidence, `--live` 사용 시 npm/GitHub fact를 gate합니다. Live output은 stale channel 보정이 명확히 드러나도록 구조화된 npm `next` dist-tag action을 포함합니다. |
...
| Evidence operations | `codexus evidence status/check/export` | App-instance observation, deterministic wiki check status, LSP detection status를 집계하는 read-only dashboard, 기존 repo/wiki/LSP/release gate를 묶는 aggregate gate report, 명시적 markdown/JSON evidence bundle export입니다. 이 명령들은 기존 evidence 위 projection이며 semantic completion이나 source-truth authority를 만들지 않습니다. |
