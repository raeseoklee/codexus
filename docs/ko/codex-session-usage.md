...
설계 방향: Codexus는 OMX-like session-native runtime으로 이동합니다. 설치된 skill은
그 첫 layer입니다. 이후 setup은 marker-bounded AGENTS guidance, session state,
checkpoint/verification command, optional hook/status/tmux integration을 추가합니다.
...
Session-native command surface가 추가되면 nested `cx run` sub-run을 시작하기 전에
checkpoint와 session verification command를 우선 사용합니다.
