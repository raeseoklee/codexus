...
layer이고, `cx setup codex-session`은 marker-bounded AGENTS guidance와
`.codex-harness/session/` state를 추가합니다. 명시적 checkpoint와 verification
command는 지금 사용할 수 있으며, hook/status/tmux integration은 아직 capability
gate 뒤에 있습니다.
