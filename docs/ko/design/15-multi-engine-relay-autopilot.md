...
상태: experimental recorder/checker 첫 slice 구현 완료. Relay adapter
descriptor/status reporting도 구현됐습니다. Active relay execution과 active external
engine adapter는 계속 deferred 0.2/0.3 설계 작업입니다.
...
이 surface는 `stability: "experimental"`이며 recorder/checker 전용입니다. 외부에서
생성된 author/reviewer artifact를 import하고, stage-gate evidence를 기록하며, convergence
agreement를 검증하고, verification 실패 시 convergence가 완료를 만들 수 없음을 증명합니다.
두 번째 engine을 spawn하지 않습니다.

향후 active relay 제안 surface:
...
구현된 slice에서 `review-engine`은 지원되는 active adapter가 이미 존재하지 않는 한
**artifact import only**를 뜻합니다. `cx autopilot relay adapters --json`은 이제
descriptor-backed adapter status를 보고합니다. Author/reviewer artifact import는
구현됐고, Codex exec, external review-engine, MCP/protocol relay driver는 unavailable로
남아 있습니다. Codexus는 외부에서 생성된 review artifact를 기록하고 shape를 검증할 수
있지만, active adapter가 구현되고 gate되기 전에는 Claude Code, 다른 engine, MCP relay를
spawn할 수 있다고 암시하면 안 됩니다.
...
구현됐습니다. `cx autopilot relay stage-gate`는 `--verification-matrix <path>`로
...
1. 완료: relay session, stage-gate evidence, convergence-agreement schema artifact와
   focused validation.
2. 완료: `cx autopilot relay record`로 단일 author/reviewer round를 report-only로 기록.
3. 완료: 첫 review engine path는 external artifact import only
   (`driverId: "external-relay"`, `spawn: false`)입니다.
4. 완료: stage-gate evidence는 `delta-check`와 `full-gate`를 구분합니다.
5. 완료: convergence validation은 같은 stage artifact hash에 대한 양쪽 role declaration을
   요구합니다.
6. 완료: valid convergence agreement가 있어도 verification 실패 시 run을 complete할 수
   없음을 테스트로 증명합니다.
7. 완료: implementation-stage convergence는 completion 전에 acceptance-criteria to
   verification matrix를 enforce합니다.
8. 완료: `cx autopilot relay adapters --json`은 descriptor-backed relay adapter status를
   보고합니다. Artifact-import author/reviewer 경로는 구현됐고, Codex exec, external
   review-engine, MCP/protocol relay driver는 spawn, source-mutation, transcript,
   verification, hook, completion authority 없이 unavailable로 남아 있습니다.
9. Deferred: 그 다음에만 active external relay adapter를 검토합니다.
