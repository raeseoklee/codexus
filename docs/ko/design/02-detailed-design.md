...
현재 structural replay는 fixture-backed pass/failure case를 포함합니다. Tool
success, permission branch, tool denial, multi-tool turn, large output, usage
accounting label에 더해 skill path, file-tool roundtrip, shell output,
interruption label의 deterministic 처리를 검증하지만, 이것은 live model
behavior parity 증명이 아니라 structural coverage입니다.
