...
`cx cancel <run-id>`는 실행 중인 supervised run에 cancel-request marker를
기록합니다. Live owner는 `owner.json` heartbeat를 갱신하고 marker를 polling한 뒤
자신의 `AbortSignal`을 중단하여 terminal `cancelled` ledger를 직접 씁니다. Owner가
dead/stale이면 canceller가 `run.cancel_orphaned`와 `run.terminal` event를 남기고
orphan ledger를 `cancelled`로 닫습니다.
...
commands, verification repair budget, driver-failure repair budget, evolution
policy, optional interop preference, automation gates를 포함합니다. Local
`codex exec` capability probe가 지원하지 않는 option은 조용히 무시하지 않고
`config.option_ignored` ledger event로 기록합니다.
