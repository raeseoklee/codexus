...
작성일: 2026-05-30
상태: recorder, completion handoff, bridge probe, launcher contract 구현됨; active spawn은 deferred
...
- Active spawning command는 capability-gated입니다. `session subagent probe --record`는
  현재 local CLI bridge를 `unavailable`로 기록하고, `launch`는 `unavailable`과 recovery
  hint가 있는 launcher-contract evidence만 기록합니다.
...
7. 구현됨: `cx session subagent probe --record --json`을 bridge-availability evidence
   artifact로 추가했습니다. 현재 local CLI boundary를 `unavailable`로 기록하며 native
   spawn support를 주장하지 않습니다.
8. Deferred: 지원되는 Codex bridge가 생길 때까지 native subagent spawning과 parallel
   planning 보류.
