...
선언된 scope가 없으면 scope finding을 **만들지 않습니다**(날조 금지). 첫 슬라이스는 scope
레인 없이 출시해도 됩니다(First Slice 참조).
...
working-tree diff와 기존 세션 verification/evidence 모델을 읽어 보수적으로 다음만 보고하는
`cx slop check --json`:
...
그리고 derivable 사실만 반영하는 `changeEvidence` tri-state 요약을 `cx session status`에 부착.
선언 scope 레인은 deferred — `--scope`(또는 `cx session intent`)가 생긴 뒤 출시해, 선언 없이
out-of-scope finding이 날조되지 않게 함.
