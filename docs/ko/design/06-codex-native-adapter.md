...
- `cx update check --json`은 explicit experimental command입니다;
- npm `latest` dist-tag 조회는 TTL cache와 짧은 timeout으로 bounded됩니다;
- npm `next`/prerelease 조회는 명시적 opt-in인
  `cx update check --channel next --json`으로만 사용할 수 있습니다;
- `version --json`, `doctor --json`, `session status --json`은 registry를 조회하지
  않는 additive cache-only `update` summary를 노출합니다;
- 사용자가 Codex session 안에서 `$codexus`를 호출하면 skill은
  `update.status: "available"`을 현재 Codex chat에 advisory로 요약할 수 있습니다;
- non-JSON `version`, `doctor`, `session status`, `session hud`는 fresh cache가
  update 가능성을 증명할 때만 짧은 advisory notice를 표시합니다;
- update check가 registry에 접근하지 못해도 primary command는 실패하지 않습니다.
...
- update notification은 informational only이며 completion, verification, release
  gate에 영향을 주지 않습니다;
- prerelease/`next` check는 명시적 opt-in일 때만 수행하고 별도 cache를 사용합니다;
- 자동 설치는 이 slice의 범위 밖입니다.
- 오래되었거나 없는 cache entry는 현재 설치본이 최신이라고 암시하지 않고 조용히
  유지되어야 합니다.
...
첫 Codex plugin package는 이제 `codex/plugins/codexus` 아래 experimental packaging
surface로 포함되며, `cx plugin status --json`은 packaged manifest, skill, wrapper
script를 검증합니다. 다만 plugin 설치 자체를 always-on 동작의 증거로 취급하면 안 됩니다.
로컬 Codex plugin 형태 기준으로 plugin은 skill, script, asset, MCP/app descriptor,
marketplace metadata를 묶을 수 있습니다. 이는 배포와 발견성을 개선하지만 기존 always-on
evidence source를 대체하지 않습니다:
...
2. `cx plugin status --json`은 experimental report-only surface로 유지합니다. 이 명령은
   packaged plugin file을 검증하지만 설치, 변경, runtime attachment 추론을 하지 않습니다.
3. 추측 없이 문서화된 Codex plugin install location을 `cx doctor --json`이 진단할 수 있을
   때까지 installed-plugin 상태는 deferred로 둡니다.
