...
- Observability adapters: app-instance observation descriptors now exist for
  ...
  identity. Next work is optional live Browser/DevTools capture drivers while
  keeping stack-specific behavior outside the workflow kernel.
- Worktree app instance launcher: [doc 19](design/19-worktree-app-instance-launcher.md)
  ...
  is optional live Browser/DevTools capture drivers and worktree-aware launcher
...
- Compiled repository wiki: [doc 18](design/18-compiled-repository-wiki.md)
  ...
  also implemented. Next work is architecture/decision/risk page coverage and
...
12. Compiled repository wiki: the deterministic `cx wiki
    ...
    context before use. Graph/session projection pages are implemented. Next
    implement architecture/decision/risk page coverage before any injection path
    is considered.
13. Worktree app instance launcher: build on the implemented live ownership
    ...
    binding adapter are implemented; next add optional live Browser/DevTools
    capture drivers and worktree-aware launcher reuse for future autopilot
    surfaces.
