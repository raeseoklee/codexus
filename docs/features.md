...
Status: current as of `main` after `0.2.7`
...
| Release integrity | `codexus release check --gate --json` | Gates install script default channel, expected-version guard, non-destructive installer help handling, trusted-publishing workflow, release evidence, and live npm/GitHub facts when `--live` is used. Stable live sign-off checks npm `latest`; npm `next` is prerelease-only and reported as not applicable for stable completion. |
...
| Evidence operations | `codexus evidence status/check/export` | Read-only project evidence dashboard, aggregate gate report over existing repo/wiki/LSP/release gates, and explicit markdown/JSON evidence bundle export. These commands are projections over existing evidence; they do not create semantic completion or source-truth authority. |
