...
`);
  ...
  assert(lsp.stability === "stable", "installed lsp check did not report stable JSON stability");
  assert(lsp.limits?.timeoutMs === 60000, "installed lsp check did not report the stable default timeout");
  assert(lsp.limits?.outputTailLimit === 4000, "installed lsp check did not report the stable output tail bound");
  ...
  const releaseCheck = parseJsonRun(codexus, ["release", "check", "--gate", "--json"]);
  assert(releaseCheck.stability === "stable", "release check local mode did not report stable JSON stability");
  assert(releaseCheck.live === false, "release check local mode should not run live release sign-off");
  assert(releaseCheck.releaseIntegrity?.status === "pass", "installed release integrity gate did not pass");
  assert(releaseCheck.gate?.status === "passed", "installed release integrity gate did not report passed");
