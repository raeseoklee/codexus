...
interface AppServerEvidenceSummary {
  path: string;
...
async function collectAppServerObserverEvidence(cwd: string): Promise<{
...
async function appServerObserverStatus(cwd: string) {
...
export async function appServerCommand(args: ParsedArgs): Promise<void> {
  ...
  if (topic === "observer") {
    ...
    if (action !== "status") throw new Error(`unsupported_app_server_observer_command:${action}`);
    const result = await appServerObserverStatus(cwd);
    if (json) {
      console.log(JSON.stringify(result, null, 2));
      return;
    }
    console.log(`app-server observer bridge: ${result.observerBridge.status}`);
    console.log(result.observerBridge.caveat);
    return;
  }
